///////////////////////////////////////////////////////////////////////////////
//
// Greasemonkey user script for FireFox and Extension script for Apple Safari 5   
//
...
var NetflixQueueSorter = (function () {
    ...
    function getLength(queueIdx) {
        if (queueIdx < totalQueueCount) {
            ...
            if (/^<code><b>\[\d+:\d+/.test(elt.innerHTML)) {
				...
				// 06/15/2010 Colin Faulkingham
				// Used to replace the greasemonkey GM_xmlhttpRequest object
				//
                var url = record.url;
    ...
    function sortByTitle() {
		...
		// 06/12/2010 Colin Faulkingham -  This is all FireFox local storage stuff.
		//
    ...
    function fixRatings(fixLinks, algorithm) {
        ...
        httpRequest.onreadystatechange = function() {	
		    if(httpRequest.readyState == 4){
		   		var returnData = httpRequest.responseText;
		        parseFixRatings(fixLinks,algorithm, record,httpRequest.responseText);
			}
		}
